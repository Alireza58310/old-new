...
export default {
    async fetch(request, env, ctx) {
        ...
        if (request.method === "POST" && url.pathname === "/api/deploy") {
            try {
                const { token, sourceUrl, placement } = await request.json();
                ...
                const workerName = `ze-alis-panel-${uniqueSuffix}`;
        ...
        async function startDeploy() {
            ...
            try {
                const response = await fetch('/api/deploy', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ token, sourceUrl: getSelectedSourceUrl(), placement: getSelectedPlacement() })
                });
