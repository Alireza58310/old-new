...
export default {
    async fetch(request, env, ctx) {
        ...
        if (request.method === "POST" && url.pathname === "/api/deploy") {
            try {
                const { token, sourceUrl, placement, panelType } = await request.json();
                ...
                const scriptSourceUrl = (sourceUrl && sourceUrl.trim()) || (isNetra ? DEFAULT_NETRA_SOURCE_URL : DEFAULT_SOURCE_URL);
                ...
                if (isNetra) {
                    ...
                    workerName = `netra-panel-${uniqueSuffix}`;
                    ...
                    // === مسیر زئوس: دقیقاً همون منطق قبلی، بدون هیچ تغییری ===
                    workerName = `ze-alis-panel-${uniqueSuffix}`;
    ...
    <p class="text-[11px] text-gray-500 dark:text-zinc-400 font-medium">
        ...
        کلیک کنید و توکن بسازید و آن را در کادر زیر وارد کنید.
            ...
            <div class="mb-5 p-1 rounded-2xl border border-gray-200 dark:border-amoled-border bg-gray-100 dark:bg-zinc-900/60 flex gap-1">
                <button type="button" id="panelTypeZeusBtn" onclick="setPanelType('zeus')" class="flex-1 py-2.5 rounded-xl text-sm font-bold transition">⚡ پنل زئوس</button>
                <button type="button" id="panelTypeNetraBtn" onclick="setPanelType('netra')" class="flex-1 py-2.5 rounded-xl text-sm font-bold transition">💜 پنل Netra</button>
            </div>
            <div class="relative">
...
async function updateZeusPanel(scriptName) {
    ...
    try {
        const response = await fetch('/api/do-update', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ token, scriptName, sourceUrl: getSelectedSourceUrl(), placement: getSelectedPlacement() })
        });
        const result = await response.json();
        if (result.success) {
...
async function reloadZeusPanel(scriptName) {
    ...
    try {
        ...
        // ==========================================================
        // مدیریت آدرس‌های سورس (گیت‌هاب) — ذخیره‌شده در localStorage مرورگر خود شما
        // هر نوع پنل (زئوس / Netra) لیست و پیش‌فرض جدای خودش رو داره.
        // ==========================================================
        let currentPanelType = localStorage.getItem('current_panel_type') || 'zeus';
        const PANEL_TYPE_CONFIG = {
            zeus: {
                storageKey: 'zeus_deployer_sources_v1',
                defaultSource: { label: 'پیش‌فرض (Alireza58310/old-new)', url: 'https://raw.githubusercontent.com/Alireza58310/old-new/refs/heads/main/old/zeus.js' }
            },
            netra: {
                storageKey: 'netra_deployer_sources_v1',
                defaultSource: { label: 'پیش‌فرض (netrair/netra-panel)', url: 'https://raw.githubusercontent.com/netrair/netra-panel/main/worker.js' }
            }
        };
        ...
        function updatePanelTypeUI() {
            ...
            if (zeusBtn && netraBtn) {
                zeusBtn.className = 'flex-1 py-2.5 rounded-xl text-sm font-bold transition ' + (isZeus ? 'bg-white dark:bg-amoled-card shadow text-emerald-600 dark:text-emerald-400' : 'text-gray-500 dark:text-zinc-400');
                netraBtn.className = 'flex-1 py-2.5 rounded-xl text-sm font-bold transition ' + (!isZeus ? 'bg-white dark:bg-amoled-card shadow text-purple-600 dark:text-purple-400' : 'text-gray-500 dark:text-zinc-400');
            }
            // مدیریت و آپدیت پنل‌ها (بررسی/ریست پسورد از طریق D1) فقط برای زئوس معنی داره؛
            // Netra خودش تنظیمات (UUID/پسورد/...) رو داخل همون پنل خودش (روی KV) مدیریت می‌کنه.
            if (updateBtn) updateBtn.style.display = isZeus ? '' : 'none';
        }

        function loadSourceList() {
            const cfg = PANEL_TYPE_CONFIG[currentPanelType];
            let list;
            try {
                list = JSON.parse(localStorage.getItem(cfg.storageKey) || 'null');
            } catch (e) { list = null; }
            if (!Array.isArray(list) || list.length === 0) {
                list = [cfg.defaultSource];
        ...
        function saveSourceList(list) {
            localStorage.setItem(PANEL_TYPE_CONFIG[currentPanelType].storageKey, JSON.stringify(list));
        }
        ...
        function renderSourceSelect() {
            ...
            list.forEach(item => {
                ...
                opt.value = item.url;
                opt.innerText = item.label + '  —  ' + item.url;
                select.appendChild(opt);
            });
            if (prevValue && list.some(i => i.url === prevValue)) select.value = prevValue;
        }
        ...
        async function addSourceUrlPrompt() {
            ...
            const label = result.label || cleanUrl.split('/').pop();
            const list = loadSourceList();
            if (list.some(i => i.url === cleanUrl)) {
                showToast('این آدرس قبلاً اضافه شده.', 'error');
                return;
            }
            list.push({ label, url: cleanUrl });
            saveSourceList(list);
            renderSourceSelect();
        ...
        function removeSelectedSourceUrl() {
            const select = document.getElementById('sourceUrlSelect');
            ...
            list = list.filter(i => i.url !== select.value);
            saveSourceList(list);
            renderSourceSelect();
        }
        ...
        function getSelectedSourceUrl() {
            const select = document.getElementById('sourceUrlSelect');
            return select && select.value ? select.value : PANEL_TYPE_CONFIG[currentPanelType].defaultSource.url;
        }
        ...
        const PLACEMENT_REGIONS = {
            ...
            azure: ["australiacentral","australiacentral2","australiaeast","australiasoutheast","austriaeast","belgiumcentral","brazilsouth","brazilsoutheast","canadacentral","canadaeast","centralindia","centralus","chilecentral","eastasia","eastus","eastus2","francecentral","francesouth","germanynorth","germanywestcentral","indonesiacentral","israelcentral","italynorth","japaneast","japanwest","koreacentral","koreasouth","malaysiawest","mexicocentral","newzealandnorth","northcentralus","northeurope","norwayeast","norwaywest","polandcentral","qatarcentral","southafricanorth","southafricawest","southcentralus","southeastasia","uksouth","ukwest","westcentralus","westeurope","westindia","westus","westus2","westus3"]
        };
        ...
        async function startDeploy() {
            ...
            await sleep(500);
            statusText.innerText = currentPanelType === 'netra' ? 'در حال ساخت KV Namespace...' : 'در حال ایجاد دیتابیس D1...';
            ...
            try {
                const response = await fetch('/api/deploy', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ token, sourceUrl: getSelectedSourceUrl(), placement: getSelectedPlacement(), panelType: currentPanelType })
                });
                statusText.innerText = currentPanelType === 'netra' ? 'در حال دریافت پنل Netra...' : 'در حال دریافت پنل زئوس...';
