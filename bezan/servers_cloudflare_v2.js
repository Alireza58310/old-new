...
<script>
    ...
    // کلودفلر API عمومی‌ای برای این نگاشت AWS/GCP/Azure نداره؛ این لیست بالا یه نگاشت دستیه.
    // اگه یه آدرس JSON (شامل aws/gcp/azure) این‌جا بذاری، این تابع هر ۳ روز یک‌بار چکش می‌کنه
    // و در صورت موفقیت جایگزین لیست بالا می‌شه. خالی بمونه یعنی همیشه از همین لیست ثابت (که الان کامله) استفاده می‌شه.
    const REGIONS_JSON_URL = ''; // مثال: 'https://raw.githubusercontent.com/USER/REPO/main/cf-regions.json'
    const REGIONS_CACHE_KEY = 'cf_pop_dashboard_regions_cache_v1';
    ...
    function loadCachedRegions() {
        try {
            ...
            if (cached && cached.data && cached.data.aws && cached.data.gcp && cached.data.azure) {
                awsRegions = cached.data.aws;
                gcpRegions = cached.data.gcp;
                azureRegions = cached.data.azure;
    ...
    async function refreshRegionsIfStale() {
        if (!REGIONS_JSON_URL) return;
        const lastFetch = loadCachedRegions();
        if (Date.now() - lastFetch < REGIONS_TTL_MS) return;
        try {
            ...
            if (data && data.aws && data.gcp && data.azure) {
                awsRegions = data.aws;
                gcpRegions = data.gcp;
                azureRegions = data.azure;
                localStorage.setItem(REGIONS_CACHE_KEY, JSON.stringify({ data, fetchedAt: Date.now() }));
                if (selectedCountry) renderDatacenters();
            }
        } catch (e) { /* آفلاین یا آدرس در دسترس نیست — از لیست کش‌شده/پیش‌فرض استفاده می‌شه */ }
    }

    let selectedCountry = null;

    function init() {
        loadCachedRegions(); // اگه قبلا نسخه‌ی تازه‌تری کش شده بود، همین الان جایگزین می‌شه
