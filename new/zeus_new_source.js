...
async function checkAutoRotates(env) {
	...
	try {
		...
		const res = await fetch("https://raw.githubusercontent.com/Alireza58310/old-new/refs/heads/main/ips.txt");
		if (!res.ok) return;
...
const SubscriptionService = {
	async generateText(user, host) {
		...
		const dynPath = encodeURIComponent("/stream/33333/" + (user.uuid ? user.uuid.split("-")[0] : "default"));
		const links = [];
		const m1 = decodeURIComponent("%E2%9A%A0%EF%B8%8F%D9%BE%D9%86%D9%84+%D8%B1%D8%A7%DB%8C%DA%AF%D8%A7%D9%86%D9%87+%D8%8C+%D9%86%D9%81%D8%B1%D9%88%D8%B4+%DA%A9.%D8%B5......%D8%B4%E2%9A%A0%EF%B8%8F");
		const m2 = decodeURIComponent("%F0%9F%9A%80%D8%B3%D8%A7%D8%AE%D8%AA+%D8%AA%D9%88%D8%B3%D8%B7+Alireza+Tune%F0%9F%9A%80");
		links.push("vl" + "e" + "ss://" + user.uuid + "@104.20.11.160:443?encryption=none&security=none&type=ws&host=" + host + "&path=" + dynPath + "#" + encodeURIComponent(m1));
		links.push("vl" + "e" + "ss://" + user.uuid + "@104.20.11.160:443?encryption=none&security=none&type=ws&host=" + host + "&path=" + dynPath + "#" + encodeURIComponent(m2));
        ...
        <div class="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="flex flex-row flex-wrap justify-center items-center gap-3 w-full md:w-auto">
                ...
                <div class="flex items-center gap-3 bg-gray-100 dark:bg-zinc-800/60 px-3 py-1.5 rounded-full border border-gray-200 dark:border-zinc-800/80 shadow-sm flex-shrink-0 w-fit">
                    ...
                    <a href="33333" target="_blank" rel="noopener noreferrer" class="text-sky-500 hover:text-sky-600 dark:hover:text-sky-400 transition-all transform hover:scale-125 duration-200 flex-shrink-0" title="Telegram">
                        <svg class="w-[22px] h-[22px] flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
...
function setModalState(modalId, show) {
	...
	async function checkGlobalMessage() {
		...
		function getvIeesLink(username) {
            ...
            const userFrag = (user.frag_len && user.frag_int) ? '&fragment=' + user.frag_len + ',' + user.frag_int : '';
            const links = [];
		const dynPath = encodeURIComponent("/stream/333/" + (user.uuid ? user.uuid.split("-")[0] : "default"));
		...
		const m2 = decodeURIComponent('%E2%99%A8%EF%B8%8F+%D8%B3%D8%A7%D8%AE%D8%AA+%D8%AA%D9%88%D8%B3%D8%B7+%D8%B9%D9%84%DB%8C%D8%B1%D8%B6%D8%A7+%E2%99%A8%EF%B8%8F');
...
async function loadLocations() {
    const cachedLocations = localStorage.getItem('cached_locations_list');
    let cachedActiveIata = localStorage.getItem('cached_active_iata') || '';
    ...
    try {
        ...
        if (statusRes.ok) {
            ...
            cachedActiveIata = statusData.iata || '';
            localStorage.setItem('cached_active_iata', cachedActiveIata);
        }
    } catch (err) {}
    try {
...
async function saveSettings() {
    ...
    try {
        ...
        if (response.ok) {
            localStorage.setItem('cached_active_iata', iata ? iata.toUpperCase() : '');
...
<div class="flex flex-col gap-4 mt-6 z-10">
    <div class="flex flex-wrap items-center gap-3 sm:gap-4 justify-center">
        ...
        <a href="" target="_blank" class="flex items-center gap-2 px-4 py-2 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-full shadow-sm hover:shadow-md transition text-sm font-bold text-gray-700 dark:text-zinc-300 hover:text-sky-500 dark:hover:text-sky-400 group">
...
${COMMON_TOAST_HTML}
    <script>
        ...
        function getvIeesLink() {
			...
			const dynPath = encodeURIComponent("/stream/3333/" + (u.uuid ? u.uuid.split("-")[0] : "default"));
			var links = [];
            let flagEmoji = '🌐';
