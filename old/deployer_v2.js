...
export default {
    async fetch(request, env, ctx) {
        ...
        if (request.method === "POST" && url.pathname === "/api/deploy") {
            try {
                const { token, sourceUrl } = await request.json();
                if (!token) throw new Error("توکن نمی‌تواند خالی باشد.");
                // آدرس سورس اختیاریه؛ اگه کاربر آدرس دلخواه نداده باشه، از همون آدرس پیش‌فرض قدیمی استفاده می‌شه.
        ...
        if (request.method === "POST" && url.pathname === "/api/do-update") {
            try {
                const { token, scriptName, sourceUrl } = await request.json();
...
<head>
    ...
    <title>Zeus Panel Deployer 1.4.3</title>
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex flex-col items-center justify-center p-4">
    <div id="mainCard" class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-3xl shadow-2xl p-8 relative overflow-hidden z-10">
        ...
        <div class="absolute -right-12 -bottom-12 w-40 h-40 bg-emerald-500/10 rounded-full blur-3xl pointer-events-none"></div>
            ...
            <h2 class="text-2xl font-black text-gray-900 dark:text-white mb-2">Zeus Panel Deployer 1.4.3</h2>
...
<div class="flex flex-col gap-4 mt-6 z-10">
    <div class="flex items-center gap-4 justify-center">
        ...
        <a href="https://t.me/ZEUS_PANEL_BOT" target="_blank" class="flex items-center gap-2 px-4 py-2 border border-sky-700 text-sky-500 bg-sky-900/20 hover:bg-sky-900/40 rounded-full shadow-sm hover:shadow-md transition text-sm font-bold group">
            <svg class="w-5 h-5 group-hover:scale-110 transition" viewBox="0 0 24 24" fill="currentColor">
...
async function updateZeusPanel(scriptName) {
    ...
    try {
        const response = await fetch('/api/do-update', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ token, scriptName, sourceUrl: getSelectedSourceUrl() })
        });
        const result = await response.json();
        if (result.success) {
...
async function reloadZeusPanel(scriptName) {
    ...
    try {
        ...
        // ==========================================================
        const SOURCE_STORAGE_KEY = 'zeus_deployer_sources_v1';
        const DEFAULT_SOURCE = {
            label: 'پیش‌فرض (Alireza58310/old-new)',
            url: 'https://raw.githubusercontent.com/Alireza58310/old-new/refs/heads/main/old/zeus.js'
        };

        function loadSourceList() {
            let list;
            try {
                list = JSON.parse(localStorage.getItem(SOURCE_STORAGE_KEY) || 'null');
            } catch (e) { list = null; }
        ...
        function addSourceUrlPrompt() {
            const url = window.prompt('آدرس raw گیت‌هاب فایل ورکر رو وارد کنید:\nمثال:\nhttps://raw.githubusercontent.com/user/repo/refs/heads/main/zeus.js');
            if (!url || !url.trim()) return;
            const cleanUrl = url.trim();
            if (!/^https:\/\/raw\.githubusercontent\.com\//i.test(cleanUrl)) {
                alert('⚠️ فقط آدرس‌های raw.githubusercontent.com پذیرفته می‌شن.');
                return;
            }
            const label = window.prompt('یه اسم دلخواه برای این سورس بذارید (اختیاری):', cleanUrl.split('/').pop()) || cleanUrl.split('/').pop();
            const list = loadSourceList();
        ...
        function getSelectedSourceUrl() {
            const select = document.getElementById('sourceUrlSelect');
            return select && select.value ? select.value : DEFAULT_SOURCE.url;
        }

        document.addEventListener('DOMContentLoaded', renderSourceSelect);
        ...
        async function startDeploy() {
            ...
            try {
                const response = await fetch('/api/deploy', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ token, sourceUrl: getSelectedSourceUrl() })
                });
