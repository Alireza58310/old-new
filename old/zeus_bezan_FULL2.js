...
// ==========================================================
// زمان اعتبار کش DNS: قبلاً ۵ دقیقه بود، یعنی هر ۵ دقیقه دوباره resolve می‌شد و اگه دامنه‌ی
// مقصد (مثلاً proxy IP دامنه‌ای که تو پنل «ثابت» کردید) چند تا IP مختلف داشت، ممکن بود
// IP جدیدی برگرده و انگار "IP عوض شده". با بردنش به چند ساعت، همون IP اول خیلی بیشتر ثابت می‌مونه.
const DNS_CACHE_TTL = 6 * 60 * 60 * 1000;
const DOH_RESOLVER = "https://cloudflare-dns.com/dns-query";
// اندازه هر بسته‌ی واقعی که به سوکت مقصد نوشته می‌شه؛ قبلاً ۱۶KB بود، بعد ۶۴KB شد.
// چون backpressure نرم (soft limit در ۷۰٪ صف) داریم، بردنش به ۱۲۸KB باز هم بی‌خطره و
// تعداد فراخوانی‌های write() رو برای یه حجم مشخص آپلود کمتر می‌کنه (throughput آپلود بهتر،
// overhead کمتر روی هر بسته). این عدد مستقیماً محدودیت Cloudflare Workers رو رد نمی‌کنه
// چون هنوز مرتب و به‌صورت stream نوشته می‌شه، نه یک‌جا.
const UPSTREAM_BUNDLE_TARGET_BYTES = 128 * 1024;
// سقف صف هر کانکشن: ۶MB قبلاً نقطه‌ی تعادل بود، ولی برای این‌که آپلود زیر فشار/جیتر شبکه
// «کم نیاره» و زودتر از حد به backpressure سخت نخوره، به ۱۲MB افزایش پیدا کرد.
// این عدد هنوز خیلی زیر سقف حافظه‌ی ۱۲۸MB ایزوله‌ست؛ برای این‌که با MAX_ISOLATE_CONNECTIONS
// (سقف کانکشن هم‌زمان) بالا هم امن بمونه، این سقف رو بالا بردیم نه اون سقف رو (کنترل OOM حفظ می‌مونه).
const UPSTREAM_QUEUE_MAX_BYTES = 12 * 1024 * 1024;
const UPSTREAM_QUEUE_MAX_ITEMS = 3000;
// این عدد (۳۲KB) عمداً دست نخورده باقی موند: طبق تحلیل، بزرگ‌تر کردنش مسیر fast-path
// نوشتن به سمت کلاینت رو (که برای بسته‌های کوچیک/معمولی بهینه شده) خراب می‌کنه و کمکی
// به سرعت دانلود واقعی نمی‌کنه؛ سرعت دانلود از مسیر backpressure/TCP_CONCURRENCY بهتر می‌شه، نه این عدد.
...
// برای استفاده شخصی، احتمال این‌که واقعاً صدها کانکشن هم‌زمان به ماکسیموم بافر برسن خیلی پایینه؛
// پس سقف رو بالاتر می‌بریم. این عدد سقف نظری/شیر اطمینانه، نه چیزی که تو مصرف عادی بهش برسید.
const MAX_ISOLATE_CONNECTIONS = 500;
let CURRENT_ISOLATE_CONNECTIONS = 0;
// فاصله زمانی پاک‌سازی دوره‌ای Mapهای سراسری که هرز می‌مونن (میلی‌ثانیه)
// قبلاً هر ۲ دقیقه بود؛ برای جلوگیری از انباشته‌شدن آروم حافظه تو کانکشن‌های طولانی (۱-۲ ساعته)، بیشترش کردیم.
const GLOBAL_MAP_SWEEP_INTERVAL = 30 * 1000;
// سقف سخت روی تعداد ورودی‌های DNS_CACHE؛ فقط یه شبکه اطمینان اضافه‌ست چون این کش فقط برای
// سرعته و چیز حیاتی توش نیست — اگه به هر دلیلی خیلی بزرگ بشه (مثلاً خیلی دامنه مختلف تو یه بازه کوتاه)،
// خودش رو کامل خالی می‌کنه؛ بدترین اتفاق فقط یه resolve DNS کمی کندتر برای دفعه بعده، هیچ کانکشن فعالی قطع نمی‌شه.
const DNS_CACHE_HARD_CAP = 300;
let lastGlobalSweep = 0;

// پاک‌سازی ورودی‌های هرز/قدیمی Mapهای سراسری تا زیر بار سنگین حافظه ایزوله پر نشه.
function sweepGlobalMaps() {
  const now = Date.now();
  if (now - lastGlobalSweep < GLOBAL_MAP_SWEEP_INTERVAL) return;
  lastGlobalSweep = now;

  if (DNS_CACHE.size > DNS_CACHE_HARD_CAP) {
    DNS_CACHE.clear();
  } else {
    for (const [key, exp] of DNS_CACHE.entries()) {
      if (!exp || exp.expires < now) DNS_CACHE.delete(key);
    }
  }
  // یوزرهایی که کانکشن فعال ندارن و ترافیک/کش‌شون صفره، بی‌فایده تو Map موندن
...
// تمدید خودکار حجم/ریکوئست کاربران (۳:۳۰ بامداد به‌وقت ایران معادل نیمه‌شب UTC است، ولی برای
// سادگی و پایداری، فاصله‌ی روزها از لحظه‌ی آخرین ریست به‌صورت مطلق (UTC ms) سنجیده می‌شه، نه
// یک ساعت مشخص؛ این تابع حداکثر هر ۱ ساعت یک‌بار (با قفل کش‌شده روی DB/Cache) اجرا می‌شه تا
// روی هر کلد استارت/درخواست بار اضافه نذاره.
...
// آپدیت خودکار/تایمری آی‌پی تمیز کلودفلر برای کاربرانی که auto_rotate_ip روشن دارن؛ هر بار که
// فاصله‌ی زمانی rotate_time (به دقیقه) از آخرین چرخش گذشته باشه، از لیست آی‌پی‌های اسکن‌شده
// (همون منبعی که اسکنر پنل استفاده می‌کنه) یک دسته‌ی تازه انتخاب و جایگزین می‌شه.
...
async function checkAutoRotates(env, ctx) {
  ...
  try {
    ...
    const res = await fetch("https://raw.githubusercontent.com/Alireza58310/old-new/refs/heads/main/bezan/zeus/ips.txt");
    if (!res.ok) return;
    ...
    for (const u of usersToRotate) {
      let availableIps = [];
      if (u.ip_operator === "all" || !u.ip_operator) {
...
export default {
  async fetch(request, env, ctx) {
    // همه‌چیز تو try/catch سراسری: هر exception مدیریت‌نشده (که باعث Error 1101 می‌شد)
    // اینجا گرفته می‌شه و به‌جای کرش کل ایزوله، یه پاسخ خطای معمولی برمی‌گرده.
    try {
      sweepGlobalMaps();
      ...
      if (Router.isWebSocketUpgrade(request) && url.pathname === '/aaaaaaaaa/bbbbbb') {
  ...
  async handleUserStatus(url, env) {
    ...
    try {
      ...
      const userJson = JSON.stringify({
        ...
        global_proxy_country: globalProxyCountry
      });
...
// API: ری استارت پنل (پاکسازی کش‌های داخلی حافظه)
...
// API: آپدیت خودکار پنل زئوس
    if (url.pathname === '/api/update-panel' && request.method === 'POST') {
      ...
      try {
        const githubRes = await fetch("https://raw.githubusercontent.com/Alireza58310/old-new/refs/heads/main/old/oldvvthisone.js?t=" + Date.now());
    ...
    if (url.pathname === '/api/proxy-ip') {
      if (request.method === 'POST') {
        const { proxy_ip, iata, country, frag_len, frag_int } = await request.json();
        ...
        if (country !== undefined) await env.DB.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES ('proxy_location_country', ?)").bind((country || '').toUpperCase()).run();
      ...
      if (isUserAction) {
        ...
        if (request.method === 'PUT') {
          ...
          if (body.toggle_only !== undefined) {
            ...
            const { username: new_username, limit_gb, expiry_days, limit_req, ips, tls, port, fingerprint, max_connections, block_porn, block_ads, auto_reset_vol_days, auto_reset_req_days, auto_rotate_ip, rotate_time, ip_operator, ip_count } = body;
            let finalUsername = username;
            if (new_username && new_username !== username) {
              if (!/^[a-zA-Z0-9_-]+$/.test(new_username)) {
                return new Response(JSON.stringify({ error: "نام کاربری جدید غیرمجاز است (فقط حروف انگلیسی، عدد، خط تیره و آندرلاین)" }), { status: 400, headers: { "Content-Type": "application/json; charset=utf-8" } });
              }
            ...
            await env.DB.prepare(
              "UPDATE users SET username = ?, limit_gb = ?, expiry_days = ?, limit_req = ?, ips = ?, tls = ?, port = ?, fingerprint = ?, max_connections = ?, block_porn = ?, block_ads = ?, auto_reset_vol_days = ?, auto_reset_req_days = ?, auto_rotate_ip = ?, rotate_time = ?, ip_operator = ?, ip_count = ? WHERE username = ?"
            ).bind(
              finalUsername,
              ...
              auto_rotate_ip ? 1 : 0,
              rotate_time ? parseInt(rotate_time) : 0,
              ip_operator || 'all',
              ip_count ? parseInt(ip_count) : 20,
              username
            ).run();
            return new Response(JSON.stringify({ success: true }), { headers: { "Content-Type": "application/json" } });
          }
        ...
        if (request.method === 'POST') {
          const { username, limit_gb, expiry_days, limit_req, ips, tls, port, fingerprint, max_connections, block_porn, block_ads, auto_reset_vol_days, auto_reset_req_days, auto_rotate_ip, rotate_time, ip_operator, ip_count } = await request.json();
          if (!username) {
          ...
          try {
            await env.DB.prepare(
              "INSERT INTO users (username, uuid, limit_gb, expiry_days, limit_req, ips, connection_type, tls, port, fingerprint, max_connections, block_porn, block_ads, auto_reset_vol_days, auto_reset_req_days, last_reset_vol_time, last_reset_req_time, auto_rotate_ip, rotate_time, ip_operator, ip_count, last_rotate_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
            ).bind(
              username, 
              uuid,
              limit_gb ? parseFloat(limit_gb) : null, 
...
const DbService = {
  async ensureSchema(db) {
    ...
    // قبلاً این ۸ کوئری پشت سر هم (سری) اجرا می‌شدن؛ روی هر کولد‌استارت ایزوله
    // جمع تأخیر همه‌شون رو به دوش اولین کانکشن کاربر می‌ذاشت (همون کندی/بی‌ثباتی اولیه اتصال).
    // با اجرای موازی، فقط به اندازه کندترین کوئری صبر می‌کنیم، نه مجموع همه.
    ...
    // ستون‌های ALTER TABLE به جدول users وابسته‌ن (باید بعد از CREATE اجرا بشن) ولی خودشون بین‌هم مستقلن، پس موازی:
    await Promise.allSettled([
      ...
      db.prepare("ALTER TABLE users ADD COLUMN fingerprint TEXT DEFAULT 'chrome'").run(),
      db.prepare("ALTER TABLE users ADD COLUMN max_connections INTEGER").run(),
      db.prepare("ALTER TABLE users ADD COLUMN limit_req INTEGER").run(),
      db.prepare("ALTER TABLE users ADD COLUMN used_req INTEGER DEFAULT 0").run(),
      db.prepare("ALTER TABLE users ADD COLUMN block_porn INTEGER DEFAULT 0").run(),
      db.prepare("ALTER TABLE users ADD COLUMN block_ads INTEGER DEFAULT 0").run(),
      db.prepare("ALTER TABLE users ADD COLUMN auto_reset_vol_days INTEGER DEFAULT 0").run(),
      db.prepare("ALTER TABLE users ADD COLUMN auto_reset_req_days INTEGER DEFAULT 0").run(),
      db.prepare("ALTER TABLE users ADD COLUMN last_reset_vol_time INTEGER DEFAULT 0").run(),
      db.prepare("ALTER TABLE users ADD COLUMN last_reset_req_time INTEGER DEFAULT 0").run(),
      db.prepare("ALTER TABLE users ADD COLUMN auto_rotate_ip INTEGER DEFAULT 0").run(),
      db.prepare("ALTER TABLE users ADD COLUMN rotate_time INTEGER DEFAULT 0").run(),
      db.prepare("ALTER TABLE users ADD COLUMN ip_operator TEXT DEFAULT 'all'").run(),
      db.prepare("ALTER TABLE users ADD COLUMN ip_count INTEGER DEFAULT 20").run(),
      db.prepare("ALTER TABLE users ADD COLUMN last_rotate_time INTEGER DEFAULT 0").run(),
    ]);
    schemaEnsured = true;
  },
...
// تبدیل کد دو حرفی کشور (cca2) به ایموجی پرچم با Regional Indicator Symbols.
// اگه کد نامعتبر/خالی باشه، ایموجی کره‌ی زمین (حالت AUTO) برمی‌گرده.
function getFlagEmojiText(cc) {
...
// رشته‌ی remark مشترک برای همه‌ی کانفیگ‌ها: TAG | FLAG | username
// مثلاً وقتی کشور روی آلمان ثابت شده: "DE | 🇩🇪 | majid"
// و وقتی هیچ کشوری ثابت نشده (حالت پیش‌فرض/AUTO): "AUTO | 🌐 | majid"
async function buildConfigRemark(user, env) {
  let cca2 = '';
  try {
    const row = await env.DB.prepare("SELECT value FROM settings WHERE key = 'proxy_location_country'").first();
    if (row && row.value) cca2 = String(row.value).toUpperCase().trim();
  } catch (e) {}

  const tag = cca2 && /^[A-Z]{2}$/.test(cca2) ? cca2 : 'AUTO';
  const flag = cca2 && /^[A-Z]{2}$/.test(cca2) ? getFlagEmojiText(cca2) : '🌐';
  return tag + ' | ' + flag + ' | ' + user.username;
}
...
const SubscriptionService = {
  async generateJson(user, host, env) {
    ...
    const m1 = decodeURIComponent('%E2%9A%A0%EF%B8%8F%D8%A7%DB%8C%D9%86+%D9%BE%D9%86%D9%84+%D8%AA%D9%88%D8%B3%D8%B7+Alireza+%D9%86%D9%88%D8%B4%D8%AA%D9%87+%D8%B4%D8%AF%D9%87+%D8%A7%D8%B3%D8%AA%E2%9A%A0%EF%B8%8F');
    const m2 = decodeURIComponent('%E2%99%A8%EF%B8%8F+%D9%81%D8%B1%D9%88%D8%B4%DB%8C+%D9%86%DB%8C%D8%B3%D8%AA+%D8%8C+%D9%86%D9%81%D8%B1%D9%88%D8%B4%DB%8C+%DA%A9....%DA%A9%D8%B4%E2%99%A8%EF%B8%8F');
        ...
        outbounds: [
          {
            ...
            ["stream" + "Settings"]: {
              network: "ws",
              ["ws" + "Settings"]: { host: host, path: "/aaaaaaaaa/bbbbbb" },
              security: "none"
            },
            tag: "proxy"
          },
    ...
    const baseRemark = await buildConfigRemark(user, env);
    const totalCombos = ips.length * ports.length;
    let comboIndex = 0;
    ...
    ips.forEach((ip) => {
      ports.forEach((portStr) => {
        ...
        // وقتی کاربر چند IP/پورت داره، برای جلوگیری از هم‌نام شدن کامل کانفیگ‌ها یه شماره اضافه می‌کنیم
        // ولی فرمت پایه (TAG | FLAG | username) همیشه حفظ می‌شه.
        const remark = totalCombos > 1 ? (baseRemark + ' #' + comboIndex) : baseRemark;
  ...
  async generateText(user, host, env) {
    ...
    links.push(atob('dmxlc3M6Ly8=') + user.uuid + '@0.0.0.0:1?encryption=none&security=none&type=ws&host=' + host + '&path=%2Faaaaaaaaa/bbbbbb#' + encodeURIComponent(m1));
    links.push(atob('dmxlc3M6Ly8=') + user.uuid + '@0.0.0.0:1?encryption=none&security=none&type=ws&host=' + host + '&path=%2Faaaaaaaaa/bbbbbb#' + encodeURIComponent(m2));
    ...
    ips.forEach((ip) => {
      ports.forEach((portStr) => {
        ...
        links.push(atob('dmxlc3M6Ly8=') + user.uuid + '@' + ip + ':' + portStr + '?path=%2Faaaaaaaaa/bbbbbb&security=' + tlsVal + '&encryption=none&insecure=0&host=' + host + '&fp=' + fp + '&type=ws&allowInsecure=0&sni=' + host + '#' + encodeURIComponent(remark));
      });
    });
...
async function handleVLESS(env, storedData = null, ctx = null) {
  // اگه تعداد کانکشن هم‌زمان روی این ایزوله از سقف رد بشه، کانکشن جدید رو رد می‌کنیم
  // تا حافظه ایزوله (که هر کانکشن می‌تونه چند مگابایت بافر بگیره) پر نشه و Worker کرش نکنه.
  ...
  const writeToRemote = async (chunk, allowRetry = true) => {
    ...
    //
    // نکته‌ی مهم درباره‌ی allowRetry: retryConnect یه سوکت TCP کاملاً جدید به مقصد باز می‌کنه.
    // برای اولین چند بایت (قبل از این‌که هندشیک TLS واقعی کلاینت با سرور مقصد رد و بدل بشه)
    // این retry بی‌خطره. ولی اگه بعد از میلیون‌ها بایت موفق (مثلاً وسط آپلود ۶۰ مگابایتی یوتیوب)
    // یه write خطا بده و ما بی‌سروصدا سوکت رو با یکی جدید عوض کنیم، از دید سرور مقصد این یعنی
    // یه کانکشن TCP کاملاً متفاوت و بی‌ربط — کل session سطح بالا (TLS/HTTP) که کلاینت روی این
    // تونل باز کرده بود خراب می‌شه، ولی چون از سمت ما "بی‌صدا" ترمیم شده، به‌جای یه قطعی تمیز و
    // قابل‌تشخیص برای کلاینت، یه‌جور خرابی/گیج‌کننده‌تر رخ می‌ده. برای همین بعد از اولین موفقیت،
    // دیگه retry رو غیرفعال می‌کنیم تا در صورت خطای واقعی، اتصال تمیز بسته بشه و مرورگر/برنامه‌ی
    // کلاینت (که خودش منطق resume داره، مثل آپلود یوتیوب) بتونه درست تشخیص بده و از سر بگیره.
  ...
  const processWsMessage = async (chunk) => {
    ...
    if (!isHeaderParsed) {
      ...
      try {
        ...
        // NSFW BLOCKER / ADS BLOCKER: بر اساس تنظیم هر کاربر، دامنه‌ی مقصد از یک DNS
        // خانواده‌محور/ضدتبلیغ استعلام می‌شه؛ اگه جواب 0.0.0.0 یا :: (یعنی بلاک‌شده) بود
        // کانکشن اصلاً باز نمی‌شه، وگرنه IP واقعی resolve شده جایگزین دامنه می‌شه.
          ...
          const task = (async () => {
            ...
            try {
              ...
              if (useFallback && proxyIP) {
                // race غیرفعاله چون این proxyIP همون آدرسیه که تو پنل "ثابت" کردید — نباید
                // بین چند IP مختلف رقابت بشه و هر بار یه IP متفاوت انتخاب بشه.
                s = await connectDirect(proxyIP, port, dataPayload, false);
              } else {
                throw err;
              }
            }
...
// سقف مخصوص صف "پیام‌های WS دریافت‌شده ولی هنوز پردازش‌نشده" — این جدا از UPSTREAM_QUEUE_MAX_BYTES/ITEMS بود
// و قبلاً از همون عدد ۶MB استفاده می‌کرد. مشکل: فشار برگشتی نرمی که برای صف بک‌اند اضافه کردیم
// باعث می‌شد صف جلویی این پیام‌های خام سریع‌تر پر بشه (چون کندتر خالی می‌شه) و به سقف مشترکش می‌خورد،
// و برخلاف صف بک‌اند، اینجا هیچ فشار برگشتی نرمی نبود — مستقیم کانکشن رو می‌بست.
// الان سقف مخصوص و خیلی بزرگ‌تری داره؛ چون فشار برگشتی بک‌اند خودش به‌طور طبیعی جلوی تلنبار شدن
// بی‌رویه رو می‌گیره، این سقف عملاً فقط یه شبکه‌ی اطمینان نهاییه، نه محدودیتی که زیر بار عادی بهش بخوریم.
const WS_INBOUND_QUEUE_MAX_BYTES = 48 * 1024 * 1024;
const WS_INBOUND_QUEUE_MAX_ITEMS = 12000;
  ...
  serverSock.addEventListener('message', (event) => {
    ...
    if (nextBytes > WS_INBOUND_QUEUE_MAX_BYTES || nextItems > WS_INBOUND_QUEUE_MAX_ITEMS) {
...
async function dohQuery(domain, recordType, resolverUrl) { 
  const cacheKey = `${domain}:${recordType}:${resolverUrl || 'default'}`;
  ...
  try {
    ...
    const response = await fetch(resolverUrl || DOH_RESOLVER, { 
      method: 'POST',
      headers: {
...
function createUpstreamQueue({ getWriter, releaseWriter, retryConnect, closeConnection, name = 'UpstreamQueue' }) {
  ...
  return {
    ...
    // علاوه بر این، وقتی صف به ~۷۰٪ سقفش نزدیک می‌شه، به‌جای این‌که بذاریم صف پر بشه و
    // یهو کل کانکشن با خطای overflow قطع بشه (که باعث میشد آپلود سریع بالا بره و بعد یهو قطع بشه)،
    // یه فشار برگشتی نرم اعمال می‌کنیم: چند میلی‌ثانیه صبر می‌کنیم تا صف کمی خالی بشه، بعد ادامه می‌دیم.
...
async function waitForBackpressure(ws) {
  if (typeof ws.bufferedAmount === 'number') {
    // قبلاً سقف ۲۵۶KB + poll هر ۱۰۰ms بود؛ همین ترکیب باعث الگوی پرشی (sawtooth) می‌شد:
    // بافر زود پر می‌شد، بعد کل ۱۰۰ms هیچی رد نمی‌شد، بعد یهو خالی می‌شد و دوباره پر سرعت شروع.
    // سقف بزرگ‌تر (۲MB) یعنی دیرتر وارد backpressure می‌شیم، و poll خیلی ریزتر (۱۰ms به‌جای ۱۰۰ms)
    // یعنی به‌محض خالی‌شدن یه‌کم از بافر، بلافاصله ادامه می‌دیم به‌جای این‌که منتظر یه چرخه کامل ۱۰۰ms بمونیم.
...
async function buildRaceCandidates(address, port, raceEnabled = true) {
  ...
  // وقتی race غیرفعاله (مثلاً برای proxy IP ثابت‌شده تو پنل)، به‌جای رقابت بین چند IP مختلف
  // (که هر بار می‌تونست IP متفاوتی رو برنده کنه و باعث "تغییر IP" بشه)، همیشه فقط اولین IP
  // لیست (که تا وقتی کش DNS معتبره — الان ۶ ساعت — ثابت می‌مونه) رو استفاده می‌کنیم.
  const limit = raceEnabled ? Math.max(1, TCP_CONCURRENCY | 0) : 1;
...
async function connectDirect(address, port, initialData = null, raceEnabled = true) {
  const raceCandidates = await buildRaceCandidates(address, port, raceEnabled);
  const candidates = raceCandidates || Array.from({ length: raceEnabled ? TCP_CONCURRENCY : 1 }, () => ({ hostname: address, port }));
...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* بکگراند متحرک سبک (فقط CSS، بدون تصویر/JS) که پشت کل پنل قرار می‌گیره */
        .zeus-bg-fx {
            position: fixed;
            inset: 0;
            z-index: 0;
        ...
        .zeus-bg-fx span:nth-child(1) {
            width: 32vw; height: 32vw; top: -8vw; right: -6vw;
            background: radial-gradient(circle, #3b82f6, transparent 70%);
            animation: zeusFloat1 22s ease-in-out infinite;
        }
        .zeus-bg-fx span:nth-child(2) {
            width: 28vw; height: 28vw; bottom: -10vw; left: -8vw;
            background: radial-gradient(circle, #6366f1, transparent 70%);
            animation: zeusFloat2 26s ease-in-out infinite;
        }
        .zeus-bg-fx span:nth-child(3) {
            width: 22vw; height: 22vw; top: 40%; left: 45%;
            background: radial-gradient(circle, #10b981, transparent 70%);
            animation: zeusFloat3 30s ease-in-out infinite;
        }
        .dark .zeus-bg-fx span { opacity: 0.16; }
        @keyframes zeusFloat1 {
            0%, 100% { transform: translate(0, 0) scale(1); }
            50% { transform: translate(-6vw, 8vw) scale(1.15); }
        }
        @keyframes zeusFloat2 {
            0%, 100% { transform: translate(0, 0) scale(1); }
            50% { transform: translate(8vw, -6vw) scale(1.1); }
        }
        @keyframes zeusFloat3 {
            0%, 100% { transform: translate(-50%, -50%) scale(1); }
            50% { transform: translate(-50%, -50%) scale(1.25); }
        }
        @media (prefers-reduced-motion: reduce) {
            .zeus-bg-fx span { animation: none; }
        }
        header, main, footer, .zeus-bg-fx + * { position: relative; z-index: 1; }
    </style>
</head>
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen transition-colors duration-200">
    <div class="zeus-bg-fx" aria-hidden="true"><span></span><span></span><span></span></div>
        ...
        <div class="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="flex flex-row flex-wrap justify-center items-center gap-3 w-full md:w-auto">
                ...
                <div class="flex items-center gap-3 bg-gray-100 dark:bg-zinc-800/60 px-3 py-1.5 rounded-full border border-gray-200 dark:border-zinc-800/80 shadow-sm flex-shrink-0 w-fit">
                    ...
                    <a href="https://www.youtube.com/aaaaaaaaa" target="_blank" rel="noopener noreferrer" class="text-red-500 hover:text-red-600 dark:hover:text-red-400 transition-all transform hover:scale-125 duration-200 flex-shrink-0" title="YouTube">
                        <svg class="w-[22px] h-[22px] flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
                    ...
                    <a href="https://t.me/aaaaaaaaa" target="_blank" rel="noopener noreferrer" class="text-sky-500 hover:text-sky-600 dark:hover:text-sky-400 transition-all transform hover:scale-125 duration-200 flex-shrink-0" title="Telegram">
                        <svg class="w-[22px] h-[22px] flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
            ...
            <div class="flex items-center justify-center gap-3 w-full md:w-auto mt-2 md:mt-0">
                ...
                <button onclick="restartCore()" title="ری استارت پـنـل" class="p-2 rounded-lg bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800/50 hover:bg-amber-100 dark:hover:bg-amber-900/50 transition text-amber-600 dark:text-amber-400 shadow-sm">
...
<div id="free-panel-warning-modal" class="fixed inset-0 z-[85] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border-4 border-red-500/50 rounded-3xl shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
...
<div id="custom-confirm-modal" class="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div id="custom-confirm-card" class="w-full max-w-sm bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-2xl shadow-2xl overflow-hidden p-6 text-center transform transition-all scale-95 duration-300">
        ...
        <div class="flex gap-3">
            <button id="custom-confirm-cancel" class="flex-1 py-3 bg-gray-100 hover:bg-gray-200 dark:bg-zinc-800 dark:hover:bg-zinc-700/80 text-gray-700 dark:text-zinc-300 font-bold rounded-xl text-sm transition duration-200">انصراف</button>
            <button id="custom-confirm-ok" class="flex-1 py-3 bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-700 hover:to-orange-700 text-white font-bold rounded-xl text-sm transition duration-200 shadow-md">تأیید</button>
        </div>
    </div>
</div>
        ...
        <div id="user-modal-card" class="w-full max-w-xl bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-850 rounded-2xl shadow-xl overflow-hidden transition-[opacity,transform] duration-200 opacity-0 scale-95 ease-out flex flex-col max-h-[90vh] transform-gpu" style="will-change: transform, opacity;">
            ...
            <form id="create-user-form" class="p-6 space-y-5 overflow-y-auto flex-1 overscroll-contain" style="-webkit-overflow-scrolling: touch; transform: translate3d(0,0,0); will-change: scroll-position, transform;" onsubmit="handleFormSubmit(event)">
                ...
                <div class="pt-2 border-t border-gray-100 dark:border-zinc-900">
                    ...
                    <div class="mt-3">
                        <label class="block text-[11px] font-bold text-gray-500 dark:text-zinc-400 mb-1.5 uppercase tracking-wider">پورت دلخواه (اختیاری)</label>
                        <input type="text" id="input-custom-ports" placeholder="8080 2096 5000" dir="ltr" class="w-full px-3 py-2 bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-xs font-mono text-left text-gray-800 dark:text-zinc-100 transition">
                    </div>
                </div>
                ...
                <div class="pt-4 border-t border-gray-100 dark:border-zinc-900 space-y-4">
					<div>
    					<div class="flex items-center justify-between mb-2">
        					...
        					<button type="button" onclick="openIpScannerModal()" class="px-2.5 py-1 bg-sky-50 dark:bg-sky-950/60 text-sky-600 dark:text-sky-400 hover:bg-sky-100 dark:hover:bg-sky-900/70 border border-sky-200 dark:border-sky-800 rounded-lg text-xs font-bold transition">اسکنر آی‌پی</button>
    					</div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div class="flex items-center justify-between bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl p-2.5 shadow-sm">
                            <span class="text-xs font-bold text-gray-700 dark:text-zinc-300">NSFW BLOCKER</span>
                        ...
                        <div class="flex items-center justify-between bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl p-2.5 shadow-sm">
                            <span class="text-xs font-bold text-gray-700 dark:text-zinc-300">ADS BLOCKER</span>
                            <label class="relative inline-flex items-center cursor-pointer select-none">
                    ...
                    <div class="flex flex-col gap-3 border border-gray-100 dark:border-zinc-900 p-3 rounded-xl bg-gray-50/60 dark:bg-zinc-900/40">
                        ...
                        <div id="auto-reset-inputs-container" class="grid grid-cols-2 gap-2 pt-2 border-t border-gray-100 dark:border-zinc-900 opacity-50 pointer-events-none transition-all duration-300">
                            <div>
                                ...
                                <input type="number" id="input-auto-reset-vol" min="1" placeholder="خالی = بدون تمدید" class="w-full px-2 py-1.5 bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-xs font-mono text-center text-gray-800 dark:text-zinc-100 transition" dir="ltr" disabled>
                            </div>
                            <div>
                                ...
                                <input type="number" id="input-auto-reset-req" min="1" placeholder="خالی = بدون تمدید" class="w-full px-2 py-1.5 bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-xs font-mono text-center text-gray-800 dark:text-zinc-100 transition" dir="ltr" disabled>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col gap-3 border border-gray-100 dark:border-zinc-900 p-3 rounded-xl bg-gray-50/60 dark:bg-zinc-900/40">
                        <div class="flex items-center justify-between">
                            <span class="text-[11px] font-bold text-gray-500 dark:text-zinc-400 uppercase tracking-wider">آپدیت خودکار آیپی تمیز (تایمری)</span>
                            <label class="relative inline-flex items-center cursor-pointer select-none">
                                <input type="checkbox" id="input-auto-rotate-ip-toggle" onchange="toggleAutoRotateIpInputs(this.checked)" class="sr-only peer">
                                <div class="w-9 h-5 bg-gray-200 peer-focus:outline-none rounded-full peer dark:bg-zinc-700 peer-checked:bg-green-600 transition-colors after:content-[''] after:absolute after:top-[2px] after:right-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-transform peer-checked:after:-translate-x-[18px]"></div>
                            </label>
                        </div>
                        <div id="auto-rotate-ip-inputs-container" class="grid grid-cols-3 gap-2 pt-2 border-t border-gray-100 dark:border-zinc-900 opacity-50 pointer-events-none transition-all duration-300">
                            <div>
                                <label class="block text-[10px] font-bold text-gray-500 dark:text-zinc-400 mb-1 uppercase tracking-wider">اپراتور</label>
                                <select id="input-ip-operator" class="w-full px-1.5 py-1.5 bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-[11px] font-semibold text-gray-700 dark:text-zinc-300 cursor-pointer" disabled>
                            ...
                            <div>
                                <label class="block text-[10px] font-bold text-gray-500 dark:text-zinc-400 mb-1 uppercase tracking-wider">تعداد آیپی</label>
                                <input type="number" id="input-ip-count" min="1" value="20" class="w-full px-2 py-1.5 bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-xs font-mono text-center text-gray-800 dark:text-zinc-100 transition" dir="ltr" disabled>
                            </div>
                            <div>
                                <label class="block text-[10px] font-bold text-gray-500 dark:text-zinc-400 mb-1 uppercase tracking-wider">هر چند دقیقه</label>
                                <input type="number" id="input-rotate-time" min="1" placeholder="مثلا 60" class="w-full px-2 py-1.5 bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-xs font-mono text-center text-gray-800 dark:text-zinc-100 transition" dir="ltr" disabled>
                            </div>
                        </div>
                    </div>
                </div>
...
<div id="ip-scanner-modal" class="fixed inset-0 z-[65] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-lg bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-2xl shadow-xl overflow-hidden transition-all transform duration-300 opacity-0 scale-95 ease-out flex flex-col max-h-[90vh]">
        <div class="px-6 py-4 border-b border-gray-150 dark:border-amoled-border flex justify-between items-center bg-gray-50 dark:bg-zinc-900/50 flex-shrink-0">
            ...
            <button type="button" onclick="toggleIpScannerModal(false)" class="text-gray-400 hover:text-gray-600 dark:hover:text-zinc-200">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
            </button>
        </div>
        <div class="p-5 space-y-4 overflow-y-auto flex-1">
            <div class="border border-green-200 dark:border-green-900/50 bg-green-50/50 dark:bg-green-900/10 rounded-xl p-4 shadow-sm">
                ...
                <div class="flex flex-col gap-2">
                    <div class="w-full bg-gray-100 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-lg p-2.5 text-[10px] font-mono text-left text-gray-800 dark:text-zinc-300 break-all select-all overflow-x-auto whitespace-pre-wrap max-h-24 overflow-y-auto" dir="ltr">python -c "import urllib.request; req = urllib.request.Request('https://raw.githubusercontent.com/Alireza58310/old-new/refs/heads/main/bezan/zeus/zeus-scanner.txt', headers={'User-Agent': 'Mozilla/5.0'}); exec(urllib.request.urlopen(req).read().decode('utf-8').split('---PYTH' + 'ON---')[1].split('---POWERSHELL---')[0].strip())"</div>
                    <button type="button" onclick="copyScannerCode('python -c &quot;import urllib.request; req = urllib.request.Request(\\'https://raw.githubusercontent.com/Alireza58310/old-new/refs/heads/main/bezan/zeus/zeus-scanner.txt\\', headers={\\'User-Agent\\': \\'Mozilla/5.0\\'}); exec(urllib.request.urlopen(req).read().decode(\\'utf-8\\').split(\\'---PYTH\\' + \\'ON---\\')[1].split(\\'---POWERSHELL---\\')[0].strip())&quot;', this)" class="w-full flex items-center justify-center gap-1.5 py-2 bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 text-gray-600 dark:text-zinc-300 hover:bg-gray-50 dark:hover:bg-zinc-700/80 rounded-lg text-xs font-bold transition shadow-sm">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg>
                        <span>کپی کد Pydroid</span>
                    </button>
                </div>
            </div>
            <div class="border border-blue-200 dark:border-blue-900/50 bg-blue-50/50 dark:bg-blue-900/10 rounded-xl p-4 shadow-sm">
                <div class="flex items-center gap-2 mb-2">
                ...
                <div class="flex flex-col gap-2">
                    <div class="w-full bg-gray-100 dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-lg p-2.5 text-[10px] font-mono text-left text-gray-800 dark:text-zinc-300 break-all select-all overflow-x-auto whitespace-pre-wrap max-h-24 overflow-y-auto" dir="ltr">powershell -ExecutionPolicy Bypass -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12 -bor [Net.SecurityProtocolType]::Tls13; $wc = New-Object System.Net.WebClient; $wc.Encoding = [System.Text.Encoding]::UTF8; $text = ($wc.DownloadString('https://raw.githubusercontent.com/Alireza58310/old-new/refs/heads/main/bezan/zeus/zeus-scanner.txt') -split '---POWERSHELL---')[1].Trim(); [IO.File]::WriteAllText('zeus-scanner.ps1', $text, [System.Text.Encoding]::UTF8); .\zeus-scanner.ps1"</div>
                    <button type="button" onclick="copyScannerCode('powershell -ExecutionPolicy Bypass -Command &quot;[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12 -bor [Net.SecurityProtocolType]::Tls13; $wc = New-Object System.Net.WebClient; $wc.Encoding = [System.Text.Encoding]::UTF8; $text = ($wc.DownloadString(\\'https://raw.githubusercontent.com/Alireza58310/old-new/refs/heads/main/bezan/zeus/zeus-scanner.txt\\') -split \\'---POWERSHELL---\\')[1].Trim(); [IO.File]::WriteAllText(\\'zeus-scanner.ps1\\', $text, [System.Text.Encoding]::UTF8); .\\\\zeus-scanner.ps1&quot;', this)" class="w-full flex items-center justify-center gap-1.5 py-2 bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 text-gray-600 dark:text-zinc-300 hover:bg-gray-50 dark:hover:bg-zinc-700/80 rounded-lg text-xs font-bold transition shadow-sm">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg>
        ...
        <div class="p-4 border-t border-gray-150 dark:border-amoled-border bg-gray-50 dark:bg-zinc-900/50 flex-shrink-0">
            <button type="button" onclick="toggleIpScannerModal(false)" class="w-full py-2.5 bg-gray-100 hover:bg-gray-200 dark:bg-zinc-800 dark:hover:bg-zinc-700/80 text-gray-700 dark:text-zinc-300 font-bold rounded-xl text-xs transition">بستن صفحه</button>
        </div>
    </div>
</div>
        ...
        <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-2xl shadow-xl overflow-hidden transition-all transform duration-300 opacity-0 scale-95 ease-out">
            ...
            <div class="p-6 space-y-4">
                <div>
                    ...
                    <div class="space-y-2">
                        <input type="text" id="global-location-search" oninput="filterGlobalLocations()" placeholder="جستجوی شهر، کشور یا IATA" class="w-full px-3 py-2 bg-white dark:bg-amoled-input border border-gray-300 dark:border-amoled-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-700 dark:text-zinc-200 transition">
                        <div class="relative">
    ...
    <script>
        ...
        window.toggleAutoRotateIpInputs = function(show) {
            ...
            const opSelect = document.getElementById('input-ip-operator');
            const countInput = document.getElementById('input-ip-count');
            const timeInput = document.getElementById('input-rotate-time');
            if (!container) return;
            if (show) {
                container.classList.remove('opacity-50', 'pointer-events-none');
        ...
        function openCreateModal() {
            ...
            if (typeof window.toggleAutoResetInputs === 'function') window.toggleAutoResetInputs(false);
        ...
        async function handleFormSubmit(event) {
            ...
            const customPortsArray = customPortsRaw.replace(/ +/g, ',').split(',').map(p => p.trim()).filter(p => p.length > 0 && /^[0-9]+$/.test(p));
            ...
            const block_porn = document.getElementById('input-block-porn') ? document.getElementById('input-block-porn').checked : false;
            const block_ads = document.getElementById('input-block-ads') ? document.getElementById('input-block-ads').checked : false;

            const autoResetOn = document.getElementById('input-auto-reset-toggle') ? document.getElementById('input-auto-reset-toggle').checked : false;
            const auto_reset_vol_days = autoResetOn ? (document.getElementById('input-auto-reset-vol').value || null) : null;
            const auto_reset_req_days = autoResetOn ? (document.getElementById('input-auto-reset-req').value || null) : null;
            if (autoResetOn && !auto_reset_vol_days && !auto_reset_req_days) {
            ...
            const autoRotateOn = document.getElementById('input-auto-rotate-ip-toggle') ? document.getElementById('input-auto-rotate-ip-toggle').checked : false;
            const auto_rotate_ip = autoRotateOn ? 1 : 0;
            const ip_operator = document.getElementById('input-ip-operator') ? document.getElementById('input-ip-operator').value : 'all';
            const ip_count = document.getElementById('input-ip-count') ? (document.getElementById('input-ip-count').value || 20) : 20;
            const rotate_time = document.getElementById('input-rotate-time') ? document.getElementById('input-rotate-time').value : null;
            if (autoRotateOn && !rotate_time) {
                alert('⚠️ وقتی تیک آپدیت خودکار آیپی روشن است، باید فیلد «هر چند دقیقه» را پر کنید!');
                submitButton.disabled = false;
                submitButton.innerText = isEditMode ? 'ذخیره تغییرات' : 'ایجاد کاربر';
                return;
            }
            ...
            try {
                const response = await fetch(url, {
                    method: method,
                    headers: { 'Content-Type': 'application/json' },
					body: JSON.stringify({ username, limit_gb: limit, expiry_days: expiry, limit_req: reqLimit, tls, port, ips, fingerprint, max_connections: maxConnections, block_porn, block_ads, auto_reset_vol_days, auto_reset_req_days, auto_rotate_ip, rotate_time, ip_operator, ip_count })
                });
...
async function restartCore() {
    ...
    try {
        ...
        if (res.ok && data.success) {
            ...
            if (btn) { btn.disabled = false; btn.classList.remove('animate-pulse'); }
        }
    } catch (err) {
        alert('خطا در ارتباط با سرور.');
        if (btn) { btn.disabled = false; btn.classList.remove('animate-pulse'); }
...
function openUsageWarning() {
    ...
    card.classList.add('opacity-100', 'scale-100');
        ...
        function getVlessLink(username) {
            ...
            links.push('vle' + 'ss://' + (user.uuid || '') + '@0.0.0.0:1?encryption=none&security=none&type=ws&host=' + host + '&path=%2Faaaaaaaaa/bbbbbb#' + encodeURIComponent(m1));
            links.push('vle' + 'ss://' + (user.uuid || '') + '@0.0.0.0:1?encryption=none&security=none&type=ws&host=' + host + '&path=%2Faaaaaaaaa/bbbbbb#' + encodeURIComponent(m2));

            const activeCca2 = (window._globalActiveCountry || '').toUpperCase();
            const activeTag = /^[A-Z]{2}$/.test(activeCca2) ? activeCca2 : 'AUTO';
            const activeFlag = /^[A-Z]{2}$/.test(activeCca2) ? getFlagEmoji(activeCca2) : '🌐';
            const baseRemark = activeTag + ' | ' + activeFlag + ' | ' + user.username;
            const totalCombos = ips.length * ports.length;
            let comboIndex = 0;
            ...
            ips.forEach((ip) => {
                ports.forEach((portStr) => {
                    ...
                    links.push('vle' + 'ss://' + (user.uuid || '') + '@' + ip + ':' + portStr + '?path=%2Faaaaaaaaa/bbbbbb&security=' + tlsVal + '&encryption=none&insecure=0&host=' + host + '&fp=' + fp + '&type=ws&allowInsecure=0&sni=' + host + '#' + encodeURIComponent(remark));
                });
            });
        ...
        function copyJsonConfig(encodedUsername) {
            ...
            const previewActiveCca2 = (window._globalActiveCountry || '').toUpperCase();
            const previewTag = /^[A-Z]{2}$/.test(previewActiveCca2) ? previewActiveCca2 : 'AUTO';
            const previewFlag = /^[A-Z]{2}$/.test(previewActiveCca2) ? getFlagEmoji(previewActiveCca2) : '🌐';
            const previewBaseRemark = previewTag + ' | ' + previewFlag + ' | ' + user.username;
            const previewTotalCombos = ips.length * ports.length;
            let previewComboIndex = 0;
            ...
            ips.forEach((ip) => {
              ports.forEach((portStr) => {
                ...
                const isTlsPort = tlsPorts.includes(portStr);
                const tlsVal = isTlsPort ? 'tls' : 'none';
                const remark = previewTotalCombos > 1 ? (previewBaseRemark + ' #' + previewComboIndex) : previewBaseRemark;
...
function editUser(encodedUsername) {
    ...
    const blockPornEl = document.getElementById('input-block-porn');
    if (blockPornEl) blockPornEl.checked = (user.block_porn === 1);
    const blockAdsEl = document.getElementById('input-block-ads');
    if (blockAdsEl) blockAdsEl.checked = (user.block_ads === 1);
    ...
    document.getElementById('input-auto-reset-vol').value = user.auto_reset_vol_days || '';
    document.getElementById('input-auto-reset-req').value = user.auto_reset_req_days || '';
    if (typeof window.toggleAutoResetInputs === 'function') window.toggleAutoResetInputs(hasAutoReset);

    const autoRotateToggle = document.getElementById('input-auto-rotate-ip-toggle');
    if (autoRotateToggle) autoRotateToggle.checked = (user.auto_rotate_ip === 1);
    const ipOperatorEl = document.getElementById('input-ip-operator');
    if (ipOperatorEl) ipOperatorEl.value = user.ip_operator || 'all';
    document.getElementById('input-ip-count').value = user.ip_count || 20;
    document.getElementById('input-rotate-time').value = user.rotate_time || '';
    if (typeof window.toggleAutoRotateIpInputs === 'function') window.toggleAutoRotateIpInputs(user.auto_rotate_ip === 1);

    const userPorts = String(user.port || '').split(',').map(p => p.trim());
    const predefinedPorts = tlsPorts.concat(nonTlsPorts);
    document.querySelectorAll('input[name="ports"]').forEach(cb => {
        cb.checked = userPorts.includes(cb.value);
    });
    const customPorts = userPorts.filter(p => p && !predefinedPorts.includes(p));
    ...
    toggleModal(true);
        ...
        // اسم انگلیسی کشور، برای مرتب‌سازی A-Z و نمایش تو خط اول option
        function getCountryDisplayNameEn(cc) {
            if (!cc) return '';
            try {
        ...
        // اسم فارسی کشور، برای نمایش داخل پرانتز
        ...
        function renderLocationsUI(locations, activeIata) {
            ...
            window._allLocationsList = locations;
            window._lastActiveIata = activeIata;

            // مرتب‌سازی A-Z بر اساس اسم انگلیسی کشور (نه کد cca2 خام)
            const sortedLocations = locations.slice().sort((a, b) => {
                const nameA = getCountryDisplayNameEn(a.cca2) || (a.cca2 || '');
                const nameB = getCountryDisplayNameEn(b.cca2) || (b.cca2 || '');
            ...
            sortedLocations.forEach(loc => {
                if (loc.iata && loc.city) {
                    ...
                    const nameEn = getCountryDisplayNameEn(loc.cca2) || (loc.cca2 || '');
                    const nameFa = getCountryDisplayName(loc.cca2) || (loc.cca2 || '');
                    const isSelected = loc.iata.toUpperCase() === activeIata.toUpperCase() ? 'selected' : '';
                    const label = flag + ' ' + nameEn + ' (' + nameFa + ') - ' + loc.city + ' (' + loc.iata + ')';
                    const searchBlob = (loc.iata + ' ' + loc.city + ' ' + (loc.cca2 || '') + ' ' + nameEn + ' ' + nameFa).toLowerCase();
                    html += '<option value="' + loc.iata + '" data-cca2="' + (loc.cca2 || '') + '" data-search="' + searchBlob + '" ' + isSelected + '>' + label + '</option>';
                }
            });
        ...
        function filterGlobalLocations() {
            ...
            const fullList = window._allLocationsList || [];
            const activeIata = window._lastActiveIata || '';
            if (!q) { renderLocationsUI(fullList, activeIata); return; }
            const filtered = fullList.filter(loc => loc.iata && loc.city &&
                (loc.iata + ' ' + loc.city + ' ' + (loc.cca2 || '') + ' ' + getCountryDisplayNameEn(loc.cca2) + ' ' + getCountryDisplayName(loc.cca2)).toLowerCase().includes(q));
            renderLocationsUI(filtered, activeIata);
        }
        ...
        async function loadLocations() {
            ...
            window._globalActiveCountry = localStorage.getItem('cached_active_country') || '';
            ...
            try {
                ...
                if (statusRes.ok) {
                    ...
                    window._globalActiveCountry = (statusData.country || '').toUpperCase();
                    localStorage.setItem('cached_active_country', window._globalActiveCountry);
        ...
        async function saveSettings() {
            ...
            const selectedCca2 = (select.selectedOptions[0] && select.selectedOptions[0].dataset.cca2) ? select.selectedOptions[0].dataset.cca2.toUpperCase() : '';
            ...
            try {
                ...
                if (iata) {
                    ...
                    if (ips.length > 0) {
                        // قبلاً اینجا یه IP رندوم از بین لیست انتخاب می‌شد — یعنی کشورهایی که
                        // چند تا IP داشتن (مثل مالزیا) هر بار ممکن بود IP متفاوتی بگیرن.
                        // الان همیشه اولین IP لیست رو قطعی انتخاب می‌کنیم تا هر کشوری،
                        // فارغ از تعداد IPهاش، دقیقاً مثل قبل (ژاپن) پایدار و ثابت بمونه.
                        resolvedIp = ips[0];
                    } else {
                        // این کشور اصلاً IP معتبری نداد (دامنه‌ی مخصوصش وجود نداره یا از کار افتاده).
                        // قبلاً اینجا بی‌سروصدا همون دامنه‌ی خراب ذخیره می‌شد و بعداً تو اتصال واقعی
                        // شکست می‌خورد بدون این‌که معلوم باشه چرا. الان واضح برمی‌گردیم به حالت
                        // پیش‌فرض پایدار و به کاربر خبر می‌دیم که این کشور در دسترس نیست.
                        countryResolveFailed = true;
                        resolvedIp = 'proxyip.cmliussss.net';
                    }
                }

                const finalCca2 = countryResolveFailed ? '' : selectedCca2;
                const response = await fetch('/api/proxy-ip', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ proxy_ip: resolvedIp, iata: countryResolveFailed ? '' : (iata ? iata.toUpperCase() : ''), country: finalCca2, frag_len: fragLen, frag_int: fragInt })
                });

                if (response.ok) {
                    window.globalFragLen = fragLen;
                    window.globalFragInt = fragInt;
                    window._globalActiveCountry = finalCca2;
                    localStorage.setItem('cached_active_country', finalCca2);
                    if (countryResolveFailed) {
                        alert('⚠️ این کشور در حال حاضر IP فعالی نداره و به حالت پیش‌فرض (پایدار) بازگشت.\\nیه کشور دیگه امتحان کنید.');
                    } else {
...
const UPDATE_FIX = "constsCURRENT_VERSION='d.d.d'";
		...
		async function checkForUpdates(isManual = false) {
            try {
                ...
                const res = await fetch('https://raw.githubusercontent.com/Alireza58310/old-new/refs/heads/main/old/zeusbezan.js?t=' + Date.now());
...
async function fetchIpsList() {
    try {
        const response = await fetch('https://raw.githubusercontent.com/Alireza58310/old-new/refs/heads/main/bezan/zeus/ips.txt');
...
function toggleIpScannerModal(show) {
    const modal = document.getElementById('ip-scanner-modal');
    const card = modal.querySelector('div');
    if (show) {
        modal.classList.remove('opacity-0', 'pointer-events-none');
        modal.classList.add('opacity-100', 'pointer-events-auto');
...
function copyScannerCode(text, btn) {
    navigator.clipboard.writeText(text).then(() => {
        ...
        btn.className = 'w-full flex items-center justify-center gap-1.5 py-2 bg-green-50 dark:bg-green-900/30 border border-green-500 text-green-600 dark:text-green-400 rounded-lg text-xs font-bold transition shadow-sm';
...
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen flex flex-col items-center py-12 px-4">
    <div class="w-full max-w-xl glass rounded-3xl shadow-2xl p-6 md:p-8 relative overflow-hidden">
        ...
        <div class="text-center mb-8 relative z-10">
            ...
            <h1 class="text-xl font-bold tracking-tight text-gray-900 dark:text-white mb-1">پنل زئوس - وضعیت اشتراک ارتقا یافته توسط علیرضا</h1>
...
<div class="flex items-center gap-4 mt-6 z-10">
    <a href="https://github.com/aaaaaaaaa" target="_blank" class="flex items-center gap-2 px-4 py-2 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-full shadow-sm hover:shadow-md transition text-sm font-bold text-gray-700 dark:text-zinc-300 hover:text-black dark:hover:text-white group">
    ...
    <script>
        ...
        function getFlagEmojiTextStatus(cc) {
            if (!cc || typeof cc !== 'string' || cc.length !== 2) return '🌐';
            var code = cc.toUpperCase();
            if (!/^[A-Z]{2}$/.test(code)) return '🌐';
            var points = [];
            for (var i = 0; i < code.length; i++) points.push(127397 + code.charCodeAt(i));
        ...
        function getVlessLink() {
            ...
            var links = [];
            var cca2 = (u.global_proxy_country || '').toUpperCase();
            var tag = /^[A-Z]{2}$/.test(cca2) ? cca2 : 'AUTO';
            var flag = /^[A-Z]{2}$/.test(cca2) ? getFlagEmojiTextStatus(cca2) : '🌐';
            var baseRemark = tag + ' | ' + flag + ' | ' + u.username;
            var totalCombos = ips.length * ports.length;
            var comboIndex = 0;
            ips.forEach(function(ip) {
                ports.forEach(function(portStr) {
                    ...
                    var remark = totalCombos > 1 ? (baseRemark + ' #' + comboIndex) : baseRemark;
                    links.push('vle' + 'ss://' + (u.uuid || '') + '@' + ip + ':' + portStr + '?path=%2Faaaaaaaaa/bbbbbb&security=' + tlsVal + '&encryption=none&insecure=0&host=' + host + '&fp=' + fp + '&type=ws&allowInsecure=0&sni=' + host + '#' + encodeURIComponent(remark));
                });
            });
            return links.join('\\n');
        }
