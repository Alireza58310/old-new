...
// اندازه هر بسته‌ی واقعی که به سوکت مقصد نوشته می‌شه؛ قبلاً ۱۶KB بود.
// چون الان backpressure درست (فشار نرم قبل از پر شدن صف) داریم، بزرگ‌تر کردنش به ۶۴KB
// بی‌خطره و تعداد فراخوانی‌های write() رو برای یه حجم مشخص آپلود کمتر می‌کنه (کارایی بهتر).
const UPSTREAM_BUNDLE_TARGET_BYTES = 64 * 1024;
// سقف صف هر کانکشن: ۱۶MB خیلی زیاد بود (چند کانکشن سنگین می‌تونستن کل حافظه ۱۲۸MB ایزوله رو پر کنن)
// ولی ۲MB هم برای ترافیک پرسرعت (مثلاً استریم) ممکنه باعث افت سرعت بشه چون بافر زود پر و backpressure زودتر می‌خوره.
// ۶MB یه نقطه تعادل عملیه: با سقف کانکشن هم‌زمان پایین‌تر (به‌جای بالا بردن هر دو با هم)، ریسک OOM کنترل می‌مونه.
        ...
        async function saveSettings() {
            ...
            try {
                ...
                const response = await fetch('/api/proxy-ip', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ proxy_ip: resolvedIp, iata: countryResolveFailed ? '' : (iata ? iata.toUpperCase() : ''), frag_len: fragLen, frag_int: fragInt })
                });
...
async function fetchIpsList() {
    try {
        const response = await fetch('https://raw.githubusercontent.com/Alireza58310/old-new/refs/heads/main/ips.txt');
