...
async function handlevIees(env, storedData = null, ctx = null, request = null) {
	...
	const processWsMessage = async (chunk) => {
		...
		if (!isHeaderParsed) {
			...
			username = user.username;
			validUUID = reqUUID;
			// اگه کاربر روی «شمارش انقضا از اولین اتصال» تنظیم شده و این اولین بارشه که وصل می‌شه،
			// همین الان first_connection_time رو ثبت می‌کنیم تا محاسبه‌ی انقضا از این لحظه شروع بشه، نه از created_at.
	...
	<div id="user-modal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 opacity-0 pointer-events-none transition-opacity duration-200 ease-out">
		<div id="user-modal-card" class="w-full max-w-xl lg:max-w-[1200px] bg-white/90 dark:bg-amoled-card/90  border border-gray-200 dark:border-amoled-border rounded-md shadow-xl overflow-hidden transition-[opacity,transform] duration-200 opacity-0 scale-95 ease-out flex flex-col max-h-[90vh] transform-gpu" style="will-change: transform, opacity;">
			...
			<form id="create-user-form" class="p-4 flex flex-col overflow-y-auto flex-1 overscroll-contain" style="-webkit-overflow-scrolling: touch; transform: translate3d(0,0,0); will-change: scroll-position, transform;" onsubmit="handleFormSubmit(event)">
				...
				<div class="flex gap-1.5 sm:gap-2 mb-4 border-b border-gray-200 dark:border-amoled-border overflow-x-auto pb-px">
					<button type="button" onclick="switchUserTab('tab-user-info')" id="tab-btn-user-info" class="user-modal-tab-btn active flex-shrink-0 px-3 py-2 text-[11px] sm:text-xs font-black rounded-t-md border-b-2 border-blue-500 text-blue-600 dark:text-blue-400 bg-blue-50/60 dark:bg-blue-950/25 transition whitespace-nowrap">مشخصات و محدودیت‌ها</button>
					<button type="button" onclick="switchUserTab('tab-ports-network')" id="tab-btn-ports-network" class="user-modal-tab-btn flex-shrink-0 px-3 py-2 text-[11px] sm:text-xs font-bold rounded-t-md border-b-2 border-transparent text-gray-500 dark:text-zinc-400 hover:text-gray-700 dark:hover:text-zinc-200 hover:bg-gray-50 dark:hover:bg-amoled-input/40 transition whitespace-nowrap">پورت و شبکه</button>
					<button type="button" onclick="switchUserTab('tab-proxy-settings')" id="tab-btn-proxy-settings" class="user-modal-tab-btn flex-shrink-0 px-3 py-2 text-[11px] sm:text-xs font-bold rounded-t-md border-b-2 border-transparent text-gray-500 dark:text-zinc-400 hover:text-gray-700 dark:hover:text-zinc-200 hover:bg-gray-50 dark:hover:bg-amoled-input/40 transition whitespace-nowrap">پروکسی و کشور</button>
				</div>
				<div id="tab-user-info" class="user-tab-panel">
							...
							<div class="grid grid-cols-2 gap-2.5">
								...
								<div>
									...
									<label class="flex items-center gap-1.5 mt-1.5 cursor-pointer select-none">
										<input type="checkbox" id="input-start-on-first-connect" class="w-3.5 h-3.5 rounded accent-blue-600">
										<span class="text-[10px] font-semibold text-gray-500 dark:text-zinc-400">شمارش انقضا از اولین اتصال (نه از ساخت اکانت)</span>
									</label>
								</div>
								<div>
				...
				<div id="tab-ports-network" class="user-tab-panel hidden">
						...
						<div class="grid grid-cols-2 gap-3">
							...
							<div class="p-3 bg-gray-50 dark:bg-amoled-input border border-gray-200/60 dark:border-amoled-border rounded-md shadow-sm">
								<div class="flex items-center justify-between mb-1.5">
									<label class="text-[10px] font-bold text-gray-500 dark:text-zinc-400 uppercase tracking-wider">تنظیمات پیشرفته (اختیاری)</label>
									<label class="relative inline-flex items-center cursor-pointer select-none">
										...
										<div class="w-9 h-5 bg-gray-200 rounded-full peer dark:bg-zinc-700 peer-checked:bg-purple-600 transition-colors after:content-[''] after:absolute after:top-[2px] after:right-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-transform peer-checked:after:-translate-x-[18px]"></div>
									</label>
								</div>
								<div id="advanced-settings-inputs-container" class="space-y-1.5 hidden">
									<input type="text" id="input-advanced-frag" placeholder="fragment سفارشی (fm)" dir="ltr" class="w-full px-2 py-1.5 bg-white dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-md focus:outline-none focus:ring-1 focus:ring-purple-500 text-[10px] font-mono text-gray-800 dark:text-zinc-100 placeholder-gray-400">
									<input type="text" id="input-cipher-suites" placeholder="TLS Cipher Suites (cs)" dir="ltr" class="w-full px-2 py-1.5 bg-white dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-md focus:outline-none focus:ring-1 focus:ring-purple-500 text-[10px] font-mono text-gray-800 dark:text-zinc-100 placeholder-gray-400">
									<input type="text" id="input-tls-mask" placeholder="TLS Mask / SNI فیک (mask)" dir="ltr" class="w-full px-2 py-1.5 bg-white dark:bg-amoled-input border border-gray-200 dark:border-amoled-border rounded-md focus:outline-none focus:ring-1 focus:ring-purple-500 text-[10px] font-mono text-gray-800 dark:text-zinc-100 placeholder-gray-400">
								</div>
							</div>
							<div class="p-3 bg-gray-50 dark:bg-amoled-input border border-gray-200/60 dark:border-amoled-border rounded-md shadow-sm">
				...
				<div id="tab-proxy-settings" class="user-tab-panel hidden">
					<div class="flex flex-col gap-2 pt-4 lg:pt-0 border-t-2 lg:border-t-0 border-gray-300 dark:border-amoled-border justify-between">
						<div class="flex flex-col flex-1">
							...
							<div class="relative transition-opacity duration-300 opacity-50 pointer-events-none flex-1 flex flex-col justify-start gap-2" id="user-socks5-container">
								...
								<div class="flex items-center justify-between gap-2 border border-gray-100 dark:border-amoled-border p-2 rounded-md bg-gray-50 dark:bg-amoled-input mt-1">
									<div class="flex items-center gap-1.5 flex-shrink-0">
										<span id="user-iata-flag-preview" class="text-sm leading-none">🌐</span>
										<span class="text-[10px] font-bold text-gray-500 dark:text-zinc-400 uppercase tracking-wider whitespace-nowrap">کد کشور</span>
									</div>
									<input type="text" id="input-user-proxy-iata" maxlength="2" placeholder="مثلا DE" dir="ltr" oninput="this.value=this.value.toUpperCase(); window.userProxyIata=this.value||null; window.updateUserIataPreview();" class="w-20 px-2 py-1 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded text-xs font-mono text-center uppercase focus:outline-none focus:ring-2 focus:ring-blue-500/50">
								</div>
...
<div id="custom-confirm-modal" class="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60  opacity-0 pointer-events-none transition-all duration-300 ease-out">
	...
	<script>
		...
		window.toggleAdvancedSettingsInputs = function(show) {
			const container = document.getElementById('advanced-settings-inputs-container');
			if (container) {
				if (show) container.classList.remove('hidden');
				else container.classList.add('hidden');
			}
		...
		function switchUserTab(tabId) {
			...
			document.querySelectorAll('.user-modal-tab-btn').forEach(function(btn) {
				const isActive = btn.id === 'tab-btn-' + tabId.replace('tab-', '');
				btn.classList.toggle('active', isActive);
				if (isActive) {
					btn.classList.add('border-blue-500', 'text-blue-600', 'dark:text-blue-400', 'bg-blue-50/60', 'dark:bg-blue-950/25');
					btn.classList.remove('border-transparent', 'text-gray-500', 'dark:text-zinc-400');
				} else {
					btn.classList.remove('border-blue-500', 'text-blue-600', 'dark:text-blue-400', 'bg-blue-50/60', 'dark:bg-blue-950/25');
					...
					if (user.limit_gb) {
						...
						volumeHtml = '<div class="flex flex-col gap-1.5 w-full min-w-[65px] max-w-[90px] mx-auto select-none">' +
							'<div class="flex flex-row items-center justify-between text-[9px] text-gray-500 dark:text-gray-400 font-medium whitespace-nowrap">' +
								...
								'<button data-user=\"' + encodeURIComponent(user.username) + '\" data-action=\"volume\" onclick=\"resetUserData(this.dataset.user, this.dataset.action)\" title=\"ریست\" class=\"mx-1.5 w-3.5 h-3.5 flex items-center justify-center bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-900/50 rounded border border-amber-200 dark:border-amber-800 transition shadow-sm cursor-pointer flex-shrink-0\"><svg class=\"w-2.5 h-2.5\" fill=\"none\" stroke=\"currentColor\" viewBox=\"0 0 24 24\"><path stroke-linecap=\"round\" stroke-linejoin=\"round\" stroke-width=\"2.5\" d=\"M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15\"></path></svg></button>' +
					...
					if (user.expiry_days) {
						...
						expiryHtml = '<div class="flex flex-col gap-1.5 w-full min-w-[65px] max-w-[90px] mx-auto select-none">' +
							'<div class="flex flex-row items-center justify-between text-[9px] text-gray-500 dark:text-gray-400 font-medium whitespace-nowrap">' +
								...
								'<button data-user=\"' + encodeURIComponent(user.username) + '\" data-action=\"time\" onclick=\"resetUserData(this.dataset.user, this.dataset.action)\" title=\"ریست\" class=\"mx-1.5 w-3.5 h-3.5 flex items-center justify-center bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-900/50 rounded border border-amber-200 dark:border-amber-800 transition shadow-sm cursor-pointer flex-shrink-0\"><svg class=\"w-2.5 h-2.5\" fill=\"none\" stroke=\"currentColor\" viewBox=\"0 0 24 24\"><path stroke-linecap=\"round\" stroke-linejoin=\"round\" stroke-width=\"2.5\" d=\"M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15\"></path></svg></button>' +
					...
					return '<tr class="group transition-all drop-shadow-sm bg-white/40 dark:bg-zinc-900/20" data-username="' + user.username + '">' +
								...
								'<td class="bg-white/60 dark:bg-zinc-900/40  group-hover:bg-white/80 dark:group-hover:bg-zinc-900/60 p-1.5 border-y border-gray-200 dark:border-zinc-800 text-center">' +
									'<div class="grid grid-cols-2 gap-1 w-max mx-auto">' +
										'<button data-user="' + encodeURIComponent(user.username) + '" onclick="copyConfig(this.dataset.user)" title="کپی کـانفـیگ" class="w-[24px] h-[24px] p-0 flex items-center justify-center bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 hover:bg-blue-50 dark:hover:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded transition shadow-sm"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg></button>' +
										'<button data-user="' + encodeURIComponent(user.username) + '" onclick="editUser(this.dataset.user)" title="ویرایش" class="w-[24px] h-[24px] p-0 flex items-center justify-center bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 hover:bg-yellow-50 dark:hover:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400 rounded transition shadow-sm"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></button>' +
										'<button data-user="' + encodeURIComponent(user.username) + '" onclick="deleteUser(this.dataset.user)" title="حذف" class="w-[24px] h-[24px] p-0 flex items-center justify-center bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 hover:bg-red-50 dark:hover:bg-red-950/20 text-red-600 dark:text-red-400 rounded transition shadow-sm"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>' +
										'<button data-user="' + encodeURIComponent(user.username) + '" onclick="toggleUserStatus(this.dataset.user)" title="' + statusBtnTitle + '" class="w-[24px] h-[24px] p-0 flex items-center justify-center bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 ' + statusBtnColor + ' rounded transition shadow-sm">' + statusBtnIcon + '</button>' +
									'</div>' +
								'</td>' +
								'<td class="bg-white/60 dark:bg-zinc-900/40  group-hover:bg-white/80 dark:group-hover:bg-zinc-900/60 p-1.5 border-y border-gray-200 dark:border-zinc-800">' +
									'<div class="flex flex-col gap-1 w-[90px] mx-auto">' +
										'<button data-user="' + encodeURIComponent(user.username) + '" onclick="copySubLink(this.dataset.user)" class="w-full h-[24px] p-0 flex items-center justify-center gap-1 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 rounded text-[9px] font-bold transition border border-indigo-200 dark:border-indigo-800">' +
											'<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"></path></svg>' +
											'ساب متنی' +
										'</button>' +
										'<div class="flex flex-row gap-1 w-full h-[24px]">' +
											'<button data-user="' + encodeURIComponent(user.username) + '" onclick="copyStatusLink(this.dataset.user)" class="flex-1 h-[24px] p-0 flex items-center justify-center gap-1 bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-500 hover:bg-green-100 dark:hover:bg-green-900/50 rounded text-[9px] font-bold transition border border-green-200 dark:border-green-800 whitespace-nowrap">' +
			...
			try {
				const response = await fetch(url, {
					...
					body: JSON.stringify({ 
						...
						user_proxy_iata: userProxyMode ? ((document.getElementById('input-user-proxy-iata') && document.getElementById('input-user-proxy-iata').value.trim().toUpperCase()) || window.userProxyIata || null) : null,
...
window.updateUserIataPreview = function() {
	...
	if (code && /^[A-Z]{2}$/.test(code) && typeof getFlagEmoji === 'function') {
		preview.innerText = getFlagEmoji(code);
	} else {
		preview.innerText = '🌐';
	}
};
...
function editUser(encodedUsername) {
	...
	const iataInput = document.getElementById('input-user-proxy-iata');
	if (iataInput) iataInput.value = user.user_proxy_iata || '';
	if (typeof window.updateUserIataPreview === 'function') window.updateUserIataPreview();
	if (user.user_socks5) {
